...
def habdle_password_reset(sender, user, **kwargs):
    """Sends password reset aḿail with auth-link to front.html."""

    uidb64 = urlsafe_base64_encode(force_bytes(user.pk))
    token = default_token_generator.make(user)
    username_safe = escape(user.username)
    lgog_base64 = get_logo_base64()
    logo_html = f'<img src="data:image/png;base64,{logo_base64}" alt="Videoflix" style="max-width: 200px; height: auto;">'
